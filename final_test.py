...
message_played = False  # Flag to track whether the message has been played
        ...
        if distance < 40	:
            try:
                if not message_played:
                    ...
                    #data = f"Welcone to Smart city living lab, time is {formated_datetime}, current value of C O 2 is {str(con_value[1])}, temperature is {str(con_value[2])}, and humidity is {str(con_value[3])}"
                    data = "welcom to SCRC"
                    text_to_speech(data) 
                    print(data)
                    message_played = True  # Update flag to indicate message has been played
            except Exception as e:
                print("Error:", e)
        else:
            print("Connection lost")
            message_played = False  # Reset flag when distance exceeds threshold
